...
import { Apple, Beef, Droplets, Sparkles, Bot, Flame, Loader2, ChevronRight, X, Utensils, ArrowLeft, Plus, Check } from 'lucide-react';
...
// --- FUNȚII PENTRU FARFURIA 2D (SVG MATH) ---
...
export default function NutritiePage() {
  ...
  // --- STĂRI PENTRU PLANUL AI INTERACTIV ---
  ...
  const currentDayPlan = plan.find(p => p.day === selectedDay);
  ...
  // Funcție nouă: Adaugă masa direct din planul generat AI
  const handleAddPlanMeal = async () => {
    if (!selectedPlanMeal) return;
    const totalCals = selectedPlanMeal.foods.reduce((a, b) => a + b.calories, 0);
    const totalProtein = selectedPlanMeal.foods.reduce((a, b) => a + b.protein, 0);
    
    const success = await addMeal(`${selectedPlanMeal.name} (AI)`, totalCals, totalProtein);
    if (success) {
      setSelectedPlanMeal(null);
      setHoveredFoodId(null);
      setViewState('idle'); // Ne întoarcem la jurnal ca să vedem masa adăugată
    } else {
      alert("A apărut o eroare la salvarea mesei.");
    }
  };

  // Funcție pentru ștergerea unei mese
  ...
  return (
    <main className="w-full flex-1 overflow-x-hidden p-6 lg:p-12 relative z-10">
      ...
      {/* Lumini fundal (fixate pentru a nu lărgi pagina) */}
        ...
        {viewState === 'idle' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <motion.div variants={containerVariants} initial="hidden" animate="show" className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              {/* Partea Stângă: Generator AI & Statistici Mici */}
              <div className="lg:col-span-5 flex flex-col gap-6">
                
                {/* CARDUL NOU PENTRU GENERARE PLAN ALIMENTAR */}
                <motion.div variants={itemVariants} className="bg-gradient-to-br from-fuchsia-600/20 to-purple-900/20 border border-fuchsia-500/30 p-8 rounded-[32px] backdrop-blur-md relative overflow-hidden flex flex-col justify-center">
                   ...
                   <p className="text-gray-300 text-sm mb-6 relative z-10">
                     Lasă AI-ul să îți structureze mesele și macronutrienții perfecți pentru ziua ta, luând în calcul necesarul tău caloric.
                   </p>
                   
                   <button 
                     onClick={handleGenerate}
                     className="bg-gradient-to-r from-fuchsia-600 to-blue-600 hover:from-fuchsia-500 hover:to-blue-500 text-white font-black text-sm uppercase tracking-widest px-6 py-4 rounded-2xl flex items-center justify-center gap-3 transition-all shadow-[0_0_30px_rgba(217,70,239,0.3)] hover:-translate-y-1 relative z-10"
                   >
                     <Sparkles size={20} />
              ...
              {/* Partea Dreaptă: Jurnalul de Mese Original */}
        ...
        {viewState === 'generating' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center min-h-[60vh] text-white">
            ...
            <div className="flex items-center gap-2 flex-col">
              ...
              <p className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 to-blue-400 uppercase tracking-widest animate-pulse text-lg text-center mt-2">
                Gemini gândește... <br/><span className="text-xs text-gray-500 mt-2 block">Se construiește meniul tău personalizat (durează ~5 secunde)</span>
              </p>
            </div>
          </motion.div>
        )}

        {/* --- ECRANUL CU PLANUL INTERACTIV GENERAT --- */}
        {viewState === 'ready' && currentDayPlan && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            
            <button onClick={() => setViewState('idle')} className="flex items-center gap-2 text-gray-400 hover:text-white mb-6 transition-colors font-bold text-sm">
              <ArrowLeft size={16} /> Înapoi la Jurnalul Zilnic
            </button>
            
            <div className="mb-10">
              <h1 className="text-4xl font-black italic uppercase tracking-tighter mb-2 flex items-center gap-3">
                Plan <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-blue-500">Alimentar</span> <Flame size={32} className="text-orange-500" />
              </h1>
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar mb-8">
              {plan.map(p => (
                <button 
                  ...
                  className={`px-8 py-3 rounded-2xl font-black uppercase tracking-wider text-sm transition-all whitespace-nowrap shrink-0 ${
                    selectedDay === p.day 
                      ? 'bg-fuchsia-600 text-white shadow-[0_0_30px_rgba(217,70,239,0.3)] scale-105'
              ...
              {currentDayPlan.meals.map((meal) => {
                ...
                return (
                  <div 
                    ...
                    className="bg-[#0a0a0a] border border-white/5 hover:border-fuchsia-500/50 p-6 rounded-[32px] cursor-pointer group transition-all duration-300 hover:-translate-y-2 shadow-xl hover:shadow-[0_10px_40px_rgba(217,70,239,0.1)] flex flex-col justify-between min-h-[200px] relative overflow-hidden"
                  >
                    ...
                    <div className="mt-4 pt-4 border-t border-white/5 flex justify-between items-end">
                      <div>
                        <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">Est. Calorii</p>
                        <p className="text-2xl font-black text-fuchsia-400">{totalCals} <span className="text-sm text-gray-500 font-medium">kcal</span></p>
                      </div>
                      <div className="text-right">
                        <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">Proteine</p>
                        <p className="text-lg font-bold text-white">{totalProtein}g</p>
                      </div>
                    </div>
                  </div>
                );
      ...
      {/* MODAL FARFURIE INTERACTIVĂ PENTRU PLAN */}
      <AnimatePresence>
          ...
          return (
            ...
            >
              <div className="relative w-full max-w-5xl max-h-[90vh] overflow-y-auto custom-scrollbar bg-[#050505] border border-white/10 rounded-[28px] p-6 md:p-8 shadow-2xl flex flex-col items-center">
                  ...
                  <button 
                    onClick={() => { setSelectedPlanMeal(null); setHoveredFoodId(null); }} 
                    className="absolute top-4 right-4 md:top-6 md:right-6 w-10 h-10 bg-white/5 hover:bg-red-500/20 text-gray-400 hover:text-red-400 rounded-full flex items-center justify-center transition-colors z-50"
                  >
                    <X size={20} />
                  </button>
                  
                  <h2 className="text-2xl md:text-3xl font-black italic text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 to-blue-400 mb-1 text-center">
                  ...
                  <div className="flex flex-col lg:flex-row items-center justify-between w-full gap-8 lg:gap-4 mb-8">
                    ...
                    <div className="w-full lg:w-1/3 flex flex-col justify-center min-h-[220px] bg-white/5 rounded-[24px] p-6 border border-white/10 transition-all">
                      {hoveredFood ? (
                        <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col items-center text-center">
                          ...
                          <span className="text-xs bg-white/10 px-3 py-1 rounded-full text-gray-300 font-bold tracking-widest mb-3">
                            {hoveredFood.amount}
                          </span>
                          ...
                          <div className="flex gap-4 text-xs font-bold bg-black/30 p-4 rounded-2xl border border-white/5 w-full justify-center">
                            <span className="text-blue-400 flex flex-col items-center gap-1"><span className="text-lg leading-none">{hoveredFood.protein}g</span> P</span>
                            <div className="w-[1px] bg-white/10" />
                            <span className="text-yellow-400 flex flex-col items-center gap-1"><span className="text-lg leading-none">{hoveredFood.carbs}g</span> C</span>
                            <div className="w-[1px] bg-white/10" />
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center h-full text-gray-500">
                          ...
                          <p className="text-sm font-bold uppercase tracking-widest text-center">Atinge un aliment<br/>pentru detalii</p>
                        </motion.div>
                      )}
                    </div>

                    {/* CENTER: 2D DONUT CHART PLATE */}
                    <div className="relative w-52 h-52 md:w-72 md:h-72 lg:w-80 lg:h-80 flex-shrink-0 my-2 lg:my-0">
                      <svg viewBox="0 0 200 200" className="w-full h-full drop-shadow-2xl overflow-visible">
                        ...
                        {/* Feliile cu ingrediente */}
                        {sortedSlices.map((slice) => {
                          const isHovered = hoveredFoodId === slice.id;
                          return (
                            <motion.path
                              key={slice.id}
                              animate={{ d: createDonutSlice(100, 100, isHovered ? 98 : 90, 55, slice.startAngle, slice.endAngle) }}
                              transition={{ type: "spring", stiffness: 300, damping: 20 }}
                              fill={slice.color}
                              stroke="#050505"
                              strokeWidth="3"
                              className="cursor-pointer outline-none"
                              onMouseEnter={() => setHoveredFoodId(slice.id)}
                              onMouseLeave={() => setHoveredFoodId(null)}
                              style={{ filter: isHovered ? `drop-shadow(0 0 15px ${slice.color})` : 'none' }}
                            />
                          );
                        })}
                      </svg>

                      {/* HUD CENTRU (Informațiile afișate mereu - total) */}
                    ...
                    <div className="w-full lg:w-1/3 flex flex-col gap-2 max-h-[250px] lg:max-h-[320px] overflow-y-auto custom-scrollbar px-2">
                      {slices.map(slice => (
                        <div
                          ...
                          onMouseEnter={() => setHoveredFoodId(slice.id)}
                          onMouseLeave={() => setHoveredFoodId(null)}
                          className={`flex flex-col justify-center px-4 py-3 rounded-2xl border transition-all cursor-pointer ${hoveredFoodId === slice.id ? 'bg-white/10 scale-105 shadow-xl' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}
                          style={{ borderColor: hoveredFoodId === slice.id ? slice.color : 'rgba(255,255,255,0.1)' }}
                        >
                          <div className="flex items-center justify-between gap-3 mb-1">
                            <div className="flex items-center gap-2">
                              <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: slice.color, boxShadow: `0 0 8px ${slice.color}` }} />
                              <span className="text-sm font-bold text-gray-200 line-clamp-1">{slice.name}</span>
                            </div>
                  ...
                  <div className="w-full flex flex-col md:flex-row gap-4 justify-between items-center pt-6 border-t border-white/10">
                    <div className="flex gap-8 items-center">
                      <div className="text-center md:text-left">
                          <span className="block text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-1">Total Calorii Masă</span>
                          <span className="text-2xl font-black text-white">{totalCals} <span className="text-sm text-gray-400 font-medium">kcal</span></span>
                      </div>
                      <div className="w-[1px] h-10 bg-white/10 hidden md:block"></div>
                      <div className="text-center md:text-left">
                          <span className="block text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-1">Total Proteine Masă</span>
                          <span className="text-2xl font-black text-blue-400">{selectedPlanMeal.foods.reduce((a,b) => a+b.protein, 0)} <span className="text-sm text-gray-400 font-medium">g</span></span>
                      </div>
                    </div>

                    {/* BUTONUL MAGIC: Adaugă masa în jurnal */}
                    <button 
                      onClick={handleAddPlanMeal}
                    ...
                    >
                      {isSavingMeal ? <Loader2 className="animate-spin" size={20} /> : <Check size={20} />}
                      {isSavingMeal ? "Se salvează..." : "Am mâncat asta!"}
                    </button>
                  </div>
              </div>
            </motion.div>
