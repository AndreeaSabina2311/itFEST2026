...
interface Exercise {
  id: string; // Am adăugat ID-ul aici
  name: string;
...
export default function WorkoutModal({ isOpen, onClose, burnedCalories, exercises }: WorkoutModalProps) {
  const { dailyStats } = useDashboardContext();
  const { deleteExercise } = dailyStats; // Extragem funcția de ștergere
