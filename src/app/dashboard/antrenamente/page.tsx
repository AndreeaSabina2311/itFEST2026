...
const WORKOUTS = [
  ...
  },
  {
    ...
    videoId: "qV9-2EF60DA",
    // videoUrl: "/videos/picioare_demo.mp4", // Exemplu fișier local
    recommended: false,
    reviews: [
...
export default function AntrenamentePage() {
  ...
  const handleFinishWorkout = async () => {
    ...
    // --- MOD DEMO (SIMULARE) ---
    // Ignorăm baza de date momentan pentru că nu ai acces la ea.
    // Doar simulăm că se salvează pentru ca UI-ul să funcționeze.
    setTimeout(() => {
        const caloriesValue = parseInt(selectedWorkout.calories.replace(/\D/g, '')) || 0;
        
        // --- SALVARE LOCALĂ (PENTRU DEMO) ---
        // Salvăm în browser pentru ca pagina de Progres să poată citi datele
        // chiar dacă baza de date nu funcționează.
        const newExercise = {
    ...
    }, 1500); // Așteptăm 1.5 secunde să pară real
  };
